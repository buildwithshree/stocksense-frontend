...
// Matches backend's ErrorResponse record (dto/response/ErrorResponse.java)
// exactly — @JsonInclude(NON_NULL) there means `path` may be absent.
export interface ApiError { status:number; error:string; message:string; timestamp:string; path?:string; }
